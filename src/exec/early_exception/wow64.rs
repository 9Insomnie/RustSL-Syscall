...
unsafe fn get_section_from_rva(
    nt_headers: *mut IMAGE_NT_HEADERS64,
    rva: u32,
) -> Option<*mut IMAGE_SECTION_HEADER> {
    ...
    let first_section = (nt_headers as usize + optional_header_offset + size_of_optional_header)
...
pub unsafe fn return_wow64_function_pointer(module_base: *mut u8) -> Option<*mut c_void> {
    ...
    for i in 0..num_sections {
        ...
        if (*section).Name.starts_with(b".rdata") {
            ...
            let entry_count = (virtual_size as usize - std::mem::size_of::<STRING>())
