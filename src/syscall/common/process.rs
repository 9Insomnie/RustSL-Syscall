...
pub fn get_process_id_by_name(name_hash: u32) -> crate::utils::error::RslResult<u32> {
    // Allocate a large buffer (e.g., 1MB)
    let buf_size = 1024 * 1024;
    let buf_ptr = ntapi::alloc_virtual_memory(buf_size, ntapi::PAGE_READWRITE)? as *mut u8;
    ...
    loop {
        ...
        if name_len > 0 && !name_buf_ptr.is_null() {
            let name_slice =
                unsafe { std::slice::from_raw_parts(name_buf_ptr, (name_len / 2) as usize) };
