...
use crate::syscall::common::hwbp::{clear_hwbp, set_hwbp, HWBPSize, HWBPType};
use crate::syscall::common::{env::get_loaded_module_by_hash, pe::get_export_by_hash};
use crate::syscall::common::{find_syscall_instruction, get_ssn};
use std::ffi::c_void;
...
pub unsafe fn register_veh() -> crate::utils::error::RslResult<VehGuard> {
    ...
    let rtl_add_veh_addr =
        get_export_by_hash(ntdll, RTL_ADD_VEH_HASH).ok_or("VEH API not found")?;
    let rtl_add_veh: RtlAddVehFn = std::mem::transmute(rtl_add_veh_addr);

    let rtl_remove_veh_addr =
        get_export_by_hash(ntdll, RTL_REMOVE_VEH_HASH).ok_or("VEH API not found")?;
    ...
    if veh_handle.is_null() {
        return Err(crate::utils::error::RslError::Other(
            "Failed to register VEH".to_string(),
        ));
    }

    Ok(VehGuard {
        handle: veh_handle,
        remove_fn: rtl_remove_veh,
    })
}

pub unsafe fn execute_with_hwbp(
    ntdll: *mut u8,
    func_hash: u32,
    callback: &mut dyn FnMut(*mut u8) -> i32,
) -> crate::utils::error::RslResult<()> {
    let func_addr = unsafe { get_export_by_hash(ntdll, func_hash) }
        .ok_or(format!("API {:#x} not found", func_hash))?;

    let hook_addr = (func_addr as usize) + 3;
    unsafe {
        set_hwbp(0, hook_addr, HWBPType::Execute, HWBPSize::Byte);
    }
...
pub unsafe extern "system" fn exception_handler(exception_info: *mut EXCEPTION_POINTERS) -> i32 {
