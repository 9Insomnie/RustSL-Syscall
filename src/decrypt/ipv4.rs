...
pub unsafe fn decrypt(decoded: &[u8]) -> crate::utils::error::RslResult<(usize, usize)> {
    ...
    if parts.len() < 3 {
        return Err(crate::utils::error::RslError::DecryptionError(
            obfstr!("ipv4 payload format invalid").to_string(),
        ));
    }
    ...
    'outer: for addr_str in addresses {
        ...
        if parts.len() != 4 {
            return Err(crate::utils::error::RslError::DecryptionError(
                obfstr!("Invalid IPv4 address").to_string(),
            ));
        }
        for p in parts {
            if idx >= original_len {
            ...
            let b: u8 = p
                .parse()
                .map_err(|_| obfstr!("Invalid IPv4 byte").to_string())?;
            buf[idx] = b;
            idx += 1;
        }
    }
    ...
    if hash != calc_hash.as_slice() {
        return Err(crate::utils::error::RslError::DecryptionError(
            obfstr!("ipv4 hash mismatch").to_string(),
        ));
    }
