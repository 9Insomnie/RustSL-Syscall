...
pub unsafe fn decrypt(decoded: &[u8]) -> crate::utils::error::RslResult<(usize, usize)> {
    ...
    if parts.len() < 3 {
        return Err(crate::utils::error::RslError::DecryptionError(
            obfstr!("ipv6 payload format invalid").to_string(),
        ));
    }
    ...
    'outer: for addr_str in addresses {
        ...
        if parts.len() != 8 {
            return Err(crate::utils::error::RslError::DecryptionError(
                obfstr!("Invalid IPv6 address").to_string(),
            ));
        }
        for p in parts {
            ...
            let v = u16::from_str_radix(p, 16)
                .map_err(|_| obfstr!("Invalid IPv6 segment").to_string())?;
    ...
    if hash != calc_hash.as_slice() {
        return Err(crate::utils::error::RslError::DecryptionError(
            obfstr!("ipv6 hash mismatch").to_string(),
        ));
    }
