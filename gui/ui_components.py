...
def create_syscall_method_combobox():
    combo = QComboBox()
    combo.setIconSize(QSize(20, 20))

    syscall_icon = get_icon('syscall')
    manifest = load_plugins_manifest()
    methods = manifest.get('syscall_methods', [])

    for m in methods:
        combo.addItem(syscall_icon, m.get('label', m['id']), m['id'])

    default_sm = get_default_value('syscall_method')
    if default_sm:
        for i in range(combo.count()):
            if combo.itemData(i) == default_sm:
                combo.setCurrentIndex(i)
                break
