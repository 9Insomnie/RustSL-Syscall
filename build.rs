...
fn main() {
    ...
    if env::var("CARGO_FEATURE_LOAD_PAYLOAD_SEPARATE").is_ok() {
        let default_payload_address =
            env::var("RSL_DEFAULT_PAYLOAD_ADDRESS").unwrap_or_else(|_| "encrypt.bin".to_string());
        println!(
            "cargo:rustc-env=RSL_ENCRYPTED_DEFAULT_PAYLOAD_ADDRESS={}",
            simple_encrypt(default_payload_address.as_bytes())
        );
    }
    ...
    if env::var("CARGO_FEATURE_PPID_SPOOFING").is_ok() {
        let parent_process_name =
            env::var("RSL_PARENT_PROCESS_NAME").unwrap_or_else(|_| "explorer.exe".to_string());
        println!(
            "cargo:rustc-env=RSL_ENCRYPTED_PARENT_PROCESS_NAME={}",
            simple_encrypt(parent_process_name.as_bytes())
        );
    }
