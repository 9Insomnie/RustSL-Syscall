...
**免责声明**：本项目并不成熟，也不保证稳定性以及免杀性，仅供教育和研究目的使用。并且使用者应确保其行为符合当地法律法规。作者不对任何非法使用或由此产生的后果承担责任。请在受控环境中测试和使用。
...
- **Threadless Injection 技术**：用于无线程 shellcode 注入和执行，包括：
  ...
  - Early Exception Inject：早期异常注入（包括 SilentMoonwalk 技术）。
...
### 依赖与环境
- **Rust 依赖**：通过 Cargo.toml 管理，包括 rsl-macros（自定义宏）。
- **Python 依赖**：各模块有 requirements.txt 文件，GUI 和 Web 模块依赖 PyQt、Flask 等。
- **构建工具**：使用 Cargo 构建 Rust 代码，支持交叉编译（x86_64-pc-windows-msvc）。
- **许可证**：LICENSE 文件定义项目许可证。
